...
static void MTTryDashboardLaunchYouTube(void){if(gHostWindow&&gPresentation)return;
...
if([v isKindOfClass:UIView.class]&&!gHostWindow){UIWindowScene *ws=nil;for(UIScene *us in UIApplication.sharedApplication.connectedScenes){if([us isKindOfClass:UIWindowScene.class]&&[us.session.persistentIdentifier containsString:@"DBDashboard-Car"]){ws=(UIWindowScene*)us;break;}}if(ws){gHostWindow=[[UIWindow alloc]initWithWindowScene:ws];gHostWindow.frame=ws.coordinateSpace.bounds;gHostWindow.windowLevel=UIWindowLevelAlert+20;UIViewController *root=[UIViewController new];root.view.backgroundColor=UIColor.clearColor;gHostWindow.backgroundColor=UIColor.clearColor;gHostWindow.opaque=NO;root.view.opaque=NO;gHostWindow.rootViewController=root;((UIView*)v).frame=root.view.bounds;((UIView*)v).autoresizingMask=UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;[root.view addSubview:(UIView*)v];gPresentation=(UIView*)v;gHostWindow.hidden=NO;[gHostWindow makeKeyAndVisible];MTLog(@"[DUO-ATTACH] window=%@ frame=%@ presentation=%@ super=%@",gHostWindow,NSStringFromCGRect(gHostWindow.frame),v,[v superview]);dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(0.5*NSEC_PER_SEC)),dispatch_get_main_queue(),^{MTLog(@"[DUO-VISIBLE] windowHidden=%d alpha=%.2f level=%.1f presWindow=%@ presHidden=%d presAlpha=%.2f layer=%@",gHostWindow.hidden,gHostWindow.alpha,gHostWindow.windowLevel,((UIView*)v).window,((UIView*)v).hidden,((UIView*)v).alpha,((UIView*)v).layer);});}}});}@catch(NSException*e){MTLog(@"[DUO-PRIMITIVE] ERROR %@ %@",e.name,e.reason);}});}
