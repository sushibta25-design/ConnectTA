...
if([v isKindOfClass:UIView.class]&&!gPresentation){UIWindow *target=nil;for(UIWindow *w in UIApplication.sharedApplication.windows){if(!w.hidden&&w.bounds.size.width>400&&w.bounds.size.height>200){target=w;break;}}if(target){UIView *host=target.rootViewController.view?:target;((UIView*)v).frame=host.bounds;((UIView*)v).autoresizingMask=UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;[host addSubview:(UIView*)v];gPresentation=(UIView*)v;MTLog(@"[DUO-INPLACE] targetWindow=%@ host=%@ presentation=%@ frame=%@",target,host,v,NSStringFromCGRect(((UIView*)v).frame));}}});}@catch(NSException*e){MTLog(@"[DUO-PRIMITIVE] ERROR %@ %@",e.name,e.reason);}});}
