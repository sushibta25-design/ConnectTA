...
static void MTHybridRefreshRosterAndActivate(void){
    ...
    @try{
        ...
        if(dash){
            ...
            @try{Class px=NSClassFromString(@"LSApplicationProxy");Class ic=NSClassFromString(@"DBApplicationInfo");id mp=((id(*)(id,SEL,id))objc_msgSend)(px,NSSelectorFromString(@"applicationProxyForIdentifier:"),@"com.google.Maps");id mi=mp?((id(*)(id,SEL,id))objc_msgSend)([ic alloc],NSSelectorFromString(@"initWithApplicationProxy:"),mp):nil;if(mi){id ms=nil;SEL sf=NSSelectorFromString(@"sceneForAppInfo:");if([dash respondsToSelector:sf])ms=((id(*)(id,SEL,id))objc_msgSend)(dash,sf,mi);MTLog(@"[DUO-MAPS] info=%@ existingScene=%@",mi,ms);if(ms){MTLog(@"[DUO-MAPS-SCENE] process=%@ definition=%@ settings=%@ clientSettings=%@",MTV(ms,@"clientProcess"),MTV(ms,@"definition"),MTV(ms,@"settings"),MTV(ms,@"clientSettings"));for(NSString *k in @[@"sceneViewController",@"controller",@"applicationSceneViewController",@"sceneController"]){id v=MTV(ms,k);if(v)MTLog(@"[DUO-MAPS-CTRL] key=%@ value=%@ class=%@",k,v,NSStringFromClass([v class]));}for(NSString *k in @[@"sceneControllers",@"applicationSceneViewControllers",@"controllers"]){id v=MTV(dash,k);if(v)MTLog(@"[DUO-DASH-CTRL] key=%@ value=%@",k,v);}}}}@catch(NSException*e){MTLog(@"[DUO-MAPS] ERROR %@ %@",e.name,e.reason);}}
