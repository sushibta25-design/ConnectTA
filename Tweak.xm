...
static id gNativeSceneEnvironment=nil;
static id gNativeProxyApplicationInfo=nil;
static id gDirectYouTubeUpdate=nil;
static BOOL gDirectYouTubePerformed=NO;
static BOOL gAggressiveKeepAlive=NO;
...
static UIView *gBestLivePresentation=nil;
...
static void __attribute__((unused)) MTDeepSceneKick(UIView *root){@try{MTLog(@"[DEEP-KICK] root=%@",root);NSArray *sels=@[@"_updateSceneLayer",@"_updateHostedLayer",@"_updateLayerHost",@"_refreshScene",@"_sceneDidUpdate",@"setNeedsUpdateOfScene",@"_invalidateScenePresentation",@"_updatePresentation"];for(NSString *n in sels){SEL q=NSSelectorFromString(n);for(id o in @[root?:[NSNull null],MTV(gYTController,@"scene")?:[NSNull null],gYTController?:[NSNull null]]){if(o!=(id)[NSNull null]&&[o respondsToSelector:q]){@try{MTLog(@"[DEEP-KICK] call %@ on %@",n,NSStringFromClass([o class]));((void(*)(id,SEL))objc_msgSend)(o,q);}@catch(NSException*e){MTLog(@"[DEEP-KICK] %@ threw %@",n,e.reason);}}}}[CATransaction flush];id sc=MTV(gYTController,@"scene");MTLog(@"[DEEP-KICK] after clientSettings=%@",MTV(sc,@"clientSettings"));}@catch(NSException*e){MTLog(@"[DEEP-KICK] ERROR %@ %@",e.name,e.reason);}}

static void MTRemoteHostKick(UIView *presentation){if(!presentation)return;@try{UIView *container=presentation.subviews.firstObject;id owner=nil;@try{owner=[container valueForKey:@"_scenePresenterOwner"];}@catch(...){}MTLog(@"[REMOTE-KICK] presentation=%@ container=%@ owner=%@",presentation,container,owner);NSArray *objs=@[container?:[NSNull null],owner?:[NSNull null]];NSArray *sels=@[@"setNeedsLayout",@"layoutIfNeeded",@"setNeedsDisplay",@"_updateScene",@"_updateScenePresentation",@"_updateHostedLayer",@"_updateHostLayer",@"_connectToScene",@"_attachToScene",@"_refresh",@"_refreshScene",@"_sceneDidUpdate"];for(id o in objs){if(o==(id)[NSNull null])continue;MTLog(@"[REMOTE-KICK] object=%@ class=%@",o,NSStringFromClass([o class]));if([NSStringFromClass([o class]) isEqualToString:@"_UISceneLayerHostContainerView"]){@try{id h=[o respondsToSelector:NSSelectorFromString(@"hostedLayers")]?((id(*)(id,SEL))objc_msgSend)(o,NSSelectorFromString(@"hostedLayers")):nil;id r=[o respondsToSelector:NSSelectorFromString(@"_realHostedLayers")]?((id(*)(id,SEL))objc_msgSend)(o,NSSelectorFromString(@"_realHostedLayers")):nil;id fl=[o respondsToSelector:NSSelectorFromString(@"_filteredLayersToPresent")]?((id(*)(id,SEL))objc_msgSend)(o,NSSelectorFromString(@"_filteredLayersToPresent")):nil;MTLog(@"[REMOTE-LAYERS] before hosted=%@ real=%@ filtered=%@",h,r,fl);SEL rb=NSSelectorFromString(@"_rebuildLayersForReason:withFence:");if([o respondsToSelector:rb]){MTLog(@"[REMOTE-REBUILD] CALL reason=MiniTaForce fence=nil");((void(*)(id,SEL,id,id))objc_msgSend)(o,rb,@"MiniTaForce",nil);}id h2=[o respondsToSelector:NSSelectorFromString(@"hostedLayers")]?((id(*)(id,SEL))objc_msgSend)(o,NSSelectorFromString(@"hostedLayers")):nil;id r2=[o respondsToSelector:NSSelectorFromString(@"_realHostedLayers")]?((id(*)(id,SEL))objc_msgSend)(o,NSSelectorFromString(@"_realHostedLayers")):nil;id fl2=[o respondsToSelector:NSSelectorFromString(@"_filteredLayersToPresent")]?((id(*)(id,SEL))objc_msgSend)(o,NSSelectorFromString(@"_filteredLayersToPresent")):nil;MTLog(@"[REMOTE-LAYERS] after hosted=%@ real=%@ filtered=%@",h2,r2,fl2);}@catch(NSException*e){MTLog(@"[REMOTE-REBUILD] ERROR %@ %@",e.name,e.reason);}}unsigned int mc=0;Method *ml=class_copyMethodList([o class],&mc);for(unsigned int i=0;i<mc;i++){NSString *n=NSStringFromSelector(method_getName(ml[i]));if([n rangeOfString:@"layer" options:NSCaseInsensitiveSearch].location!=NSNotFound||[n rangeOfString:@"scene" options:NSCaseInsensitiveSearch].location!=NSNotFound||[n rangeOfString:@"host" options:NSCaseInsensitiveSearch].location!=NSNotFound)MTLog(@"[REMOTE-METHOD] %@ %@",NSStringFromClass([o class]),n);}if(ml)free(ml);for(NSString *sn in sels){SEL z=NSSelectorFromString(sn);if([o respondsToSelector:z]){@try{NSMethodSignature *sig=[o methodSignatureForSelector:z];if(sig.numberOfArguments==2){MTLog(@"[REMOTE-KICK] CALL %@ %@",NSStringFromClass([o class]),sn);((void(*)(id,SEL))objc_msgSend)(o,z);}}@catch(NSException*e){MTLog(@"[REMOTE-KICK] CALL ERROR %@ %@ %@",sn,e.name,e.reason);}}}}[CATransaction flush];MTLog(@"[REMOTE-KICK] done");}@catch(NSException*e){MTLog(@"[REMOTE-KICK] ERROR %@ %@",e.name,e.reason);}}

static void MTAggressiveTick(int n){if(n>=24||!gAggressiveKeepAlive)return;dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(0.5*NSEC_PER_SEC)),dispatch_get_main_queue(),^{@try{id sc=MTV(gYTController,@"scene");BOOL fg=[[[MTV(sc,@"settings") valueForKey:@"foreground"] description] boolValue];BOOL attached=(gBestLivePresentation&&gBestLivePresentation.window!=nil);MTLog(@"[AGGR-LOOP] n=%d fg=%d attached=%d subviews=%lu",n,fg,attached,(unsigned long)gBestLivePresentation.subviews.count);if(!attached&&gBestLivePresentation&&gHostWindow){[gBestLivePresentation removeFromSuperview];gBestLivePresentation.frame=gHostWindow.bounds;[gHostWindow.rootViewController.view addSubview:gBestLivePresentation];[gHostWindow makeKeyAndVisible];MTLog(@"[AGGR-LOOP] reattached live presentation");}if(!fg&&gDirectYouTubeUpdate&&[gDirectYouTubeUpdate respondsToSelector:NSSelectorFromString(@"performUpdateWithCompletion:")]){MTLog(@"[AGGR-LOOP] FORCE update");((void(*)(id,SEL,id))objc_msgSend)(gDirectYouTubeUpdate,NSSelectorFromString(@"performUpdateWithCompletion:"),^{MTLog(@"[AGGR-LOOP] FORCE completion");MTRemoteHostKick(gBestLivePresentation);});}MTAggressiveTick(n+1);}@catch(NSException*e){MTLog(@"[AGGR-LOOP] ERROR %@ %@",e.name,e.reason);}});}

static void MTHostLiveTry(int tries){dispatch_async(dispatch_get_main_queue(),^{@try{id sc=MTV(gYTController,@"scene");id st=MTV(sc,@"settings");BOOL fg=[[MTV(st,@"foreground") description] boolValue];SEL pv=NSSelectorFromString(@"presentationViewWithIdentifier:");id v=gBestLivePresentation?:([gYTController respondsToSelector:pv]?((id(*)(id,SEL,id))objc_msgSend)(gYTController,pv,@"com.sushibta.minita.live"):nil);BOOL real=[NSStringFromClass([v class]) containsString:@"UIScenePresentationView"];NSUInteger subs=[v respondsToSelector:@selector(subviews)]?[[v subviews] count]:0;MTLog(@"[LIVE-HOST] try=%d fg=%d view=%@ real=%d subviews=%lu",tries,fg,v,real,(unsigned long)subs);if((!fg||!real||subs==0)&&tries<12){dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(0.25*NSEC_PER_SEC)),dispatch_get_main_queue(),^{MTHostLiveTry(tries+1);});return;}if(!fg||!real||subs==0){MTLog(@"[LIVE-HOST] no live presentation");return;}UIWindowScene *ws=MTCarScene();if(!ws)return;if(!gHostWindow){gHostWindow=[[UIWindow alloc]initWithWindowScene:ws];gHostWindow.frame=ws.coordinateSpace.bounds;gHostWindow.windowLevel=UIWindowLevelAlert+60;gHostWindow.rootViewController=[UIViewController new];gHostWindow.rootViewController.view.backgroundColor=UIColor.blackColor;}UIView *p=(UIView*)v;[p removeFromSuperview];p.frame=gHostWindow.bounds;p.autoresizingMask=UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;[gHostWindow.rootViewController.view addSubview:p];gPresentation=p;gHostWindow.hidden=NO;[gHostWindow makeKeyAndVisible];MTRemoteHostKick(gBestLivePresentation);if(!gAggressiveKeepAlive){gAggressiveKeepAlive=YES;MTAggressiveTick(0);MTLog(@"[AGGR-LOOP] started 12s force-foreground test");}MTLog(@"[LIVE-HOST] ATTACHED %@ frame=%@ subviews=%lu window=%@",NSStringFromClass(p.class),NSStringFromCGRect(p.frame),(unsigned long)p.subviews.count,p.window);dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(0.35*NSEC_PER_SEC)),dispatch_get_main_queue(),^{id sc=MTV(gYTController,@"scene");BOOL fg=[[[MTV(sc,@"settings") valueForKey:@"foreground"] description] boolValue];MTLog(@"[AGGR] post-attach fg=%d scene=%@",fg,sc);if(!fg&&gDirectYouTubeUpdate&&[gDirectYouTubeUpdate respondsToSelector:NSSelectorFromString(@"performUpdateWithCompletion:")]){MTLog(@"[AGGR] replay exact update to force foreground");((void(*)(id,SEL,id))objc_msgSend)(gDirectYouTubeUpdate,NSSelectorFromString(@"performUpdateWithCompletion:"),^{MTLog(@"[AGGR] replay completion");});}});}@catch(NSException*e){MTLog(@"[LIVE-HOST] ERROR %@ %@",e.name,e.reason);}});}
static void MTHostYouTubeAfterExactUpdate(void){MTHostLiveTry(0);}
...
static void __attribute__((used)) MTDumpSceneInternals(id controller){
    ...
    if(update){
        ...
        free(m);
        MTLog(@"[UPDATE] %@",update);if(update&&[update respondsToSelector:NSSelectorFromString(@"environment")]){id ee=((id(*)(id,SEL))objc_msgSend)(update,NSSelectorFromString(@"environment"));if(ee&&!gNativeSceneEnvironment){gNativeSceneEnvironment=ee;MTLog(@"[UPDATE-ENV-CAPTURE] env=%@ class=%@",ee,NSStringFromClass([ee class]));}}
    }
}
...
- (void)_handleOpenApplicationEvent:(id)event {
    ...
    id seedInfo=ctx;id nativeApp=MTV(seedInfo,@"application");if(!nativeApp){id vv=MTV(ctx,@"value");if(vv){seedInfo=vv;nativeApp=MTV(vv,@"application");}}NSString *nativeBundle=MTV(nativeApp,@"bundleIdentifier");if(!nativeBundle)nativeBundle=MTV(nativeApp,@"applicationIdentifier");MTLog(@"[DUO-SEED-CHECK] ctx=%@ seedInfo=%@ app=%@ bundle=%@",ctx,seedInfo,nativeApp,nativeBundle);if([nativeBundle isEqualToString:@"com.google.Maps"]||[nativeBundle isEqualToString:@"com.apple.Maps"]){gMTHybridNativeLaunchArg=seedInfo;gNativeDashboardSettings=[MTV(seedInfo,@"activationSettings") copy];MTLog(@"[DUO-SEED] captured native %@ launch contract settings=%@",nativeBundle,gNativeDashboardSettings);@try{id mapsScene=nil;Class SM=NSClassFromString(@"FBSceneManager");id mgr=[SM respondsToSelector:NSSelectorFromString(@"sharedInstance")]?((id(*)(id,SEL))objc_msgSend)(SM,NSSelectorFromString(@"sharedInstance")):nil;MTLog(@"[UPDATE-ENV-PROBE] mapsScene=%@ manager=%@ nativeSettings=%@",mapsScene,mgr,gNativeDashboardSettings);if(gNativeSceneEnvironment){MTLog(@"[UPDATE-ENV-READY] env=%@ settings=%@",gNativeSceneEnvironment,gNativeDashboardSettings);if(gYTAppInfo&&gNativeProxyApplicationInfo){@try{Class U=NSClassFromString(@"DBSceneUpdate");SEL z=NSSelectorFromString(@"initWithApplicationInfo:proxyApplicationInfo:environment:activationSettings:");id probe=(U&&[U instancesRespondToSelector:z])?((id(*)(id,SEL,id,id,id,id))objc_msgSend)([U alloc],z,gYTAppInfo,gNativeProxyApplicationInfo,gNativeSceneEnvironment,gNativeDashboardSettings):nil;MTLog(@"[UPDATE-CONSTRUCT] probe=%@ app=%@ proxy=%@ env=%@ activation=%@",probe,MTV(probe,@"applicationInfo"),MTV(probe,@"proxyApplicationInfo"),MTV(probe,@"environment"),MTV(probe,@"activationSettings"));gDirectYouTubeUpdate=probe;if(probe&&!gDirectYouTubePerformed&&[probe respondsToSelector:NSSelectorFromString(@"performUpdateWithCompletion:")]){gDirectYouTubePerformed=YES;MTLog(@"[UPDATE-PERFORM] begin exact YouTube/CarPlayTemplateUIHost update");void (^done)(void)=^{MTLog(@"[UPDATE-PERFORM] completion");MTHostYouTubeAfterExactUpdate();};((void(*)(id,SEL,id))objc_msgSend)(probe,NSSelectorFromString(@"performUpdateWithCompletion:"),done);}}@catch(NSException*e){MTLog(@"[UPDATE-CONSTRUCT] ERROR %@ %@",e.name,e.reason);}}}else{MTLog(@"[UPDATE-DIRECT] waiting for captured DBSceneEnvironment");}}@catch(NSException*e){MTLog(@"[UPDATE-ENV-PROBE] ERROR %@ %@",e.name,e.reason);}MTLog(@"[UPDATE-DIRECT] legacy launch suppressed");gDidLaunchYT=NO;dispatch_async(dispatch_get_main_queue(),^{MTTryDashboardLaunchYouTube();});}
    if(ctx){
...
static BOOL didCapturedRefg=NO;if(!didCapturedRefg&&[gYTSettings isKindOfClass:NSDictionary.class]&&gYTSettings[@"DBActivationSettingLaunchSource"]&&!gPresentation){didCapturedRefg=YES;NSDictionary *captured=[gYTSettings copy];id ctrl=(id)self;MTLog(@"[DUO-CAPTURED-SETTINGS] %@",captured);dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(0.25*NSEC_PER_SEC)),dispatch_get_main_queue(),^{@try{SEL fg=NSSelectorFromString(@"foregroundSceneWithSettings:completion:");if([ctrl respondsToSelector:fg])((void(*)(id,SEL,id,id))objc_msgSend)(ctrl,fg,captured,nil);MTLog(@"[DUO-CAPTURED-REFG] scene=%@ process=%@",MTV(ctrl,@"scene"),MTV(MTV(ctrl,@"scene"),@"clientProcess"));}@catch(NSException*e){MTLog(@"[DUO-CAPTURED-REFG] ERROR %@ %@",e.name,e.reason);}});}
    ...
    if(logRender) MTLog(@"[RENDER-B] presentation=%@ class=%@ window=%@ subviews=%lu",pv,NSStringFromClass([pv class]),[pv window],(unsigned long)[pv subviews].count); if(!gDirectYouTubePerformed)dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(0.25*NSEC_PER_SEC)),dispatch_get_main_queue(),^{ MTHostYouTube(); });
...
- (id)initWithApplicationInfo:(id)app proxyApplicationInfo:(id)proxy environment:(id)env activationSettings:(id)settings {
    id r=%orig;
    NSString *bid=nil; @try{bid=MTV(app,@"bundleIdentifier");} @catch(...){}
    if(env && [bid isEqualToString:@"com.google.Maps"]){gNativeSceneEnvironment=env;gNativeProxyApplicationInfo=proxy;MTLog(@"[ENV-HOOK] maps init4 env=%@ proxy=%@ settings=%@",env,proxy,settings);}
    return r;
}
%end
