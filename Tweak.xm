...
- (void)_handleOpenApplicationEvent:(id)event {
    ...
    if(ctx){
        ...
        static BOOL onceB=NO;if(!onceB){onceB=YES;Class px=NSClassFromString(@"LSApplicationProxy");Class ic=NSClassFromString(@"DBApplicationInfo");id pp=((id(*)(id,SEL,id))objc_msgSend)(px,NSSelectorFromString(@"applicationProxyForIdentifier:"),@"com.google.ios.youtube");id localYT=((id(*)(id,SEL,id))objc_msgSend)([ic alloc],NSSelectorFromString(@"initWithApplicationProxy:"),pp);MTLog(@"[TEST-B] localYT=%@",localYT);Class c=NSClassFromString(@"DBApplicationLaunchInfo");SEL z=NSSelectorFromString(@"initWithApplication:activationSettings:");id as=MTV(ctx,@"activationSettings");if(c&&[[c alloc] respondsToSelector:z]){id y=((id(*)(id,SEL,id,id))objc_msgSend)([c alloc],z,localYT,as);Class ec=NSClassFromString(@"DBEvent");id ev=[ec new];@try{[ev setValue:@4 forKey:@"type"];[ev setValue:y forKey:@"context"];}@catch(NSException*e){MTLog(@"[TEST-B] event error %@",e.reason);}MTLog(@"[TEST-B] event=%@ ctx=%@",ev,y);if(gMTHybridDashboard)((void(*)(id,SEL,id))objc_msgSend)(gMTHybridDashboard,NSSelectorFromString(@"_handleOpenApplicationEvent:"),ev);}}
