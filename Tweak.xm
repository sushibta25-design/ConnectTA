...
static void MTHostYouTubeAfterExactUpdate(void){dispatch_async(dispatch_get_main_queue(),^{@try{if(!gYTController){MTLog(@"[FINAL-HOST] no controller");return;}id sc=MTV(gYTController,@"scene");id cp=MTV(sc,@"clientProcess");MTLog(@"[FINAL-HOST] scene=%@ process=%@ settings=%@",sc,cp,MTV(sc,@"settings"));SEL pv=NSSelectorFromString(@"presentationViewWithIdentifier:");id v=[gYTController respondsToSelector:pv]?((id(*)(id,SEL,id))objc_msgSend)(gYTController,pv,@"com.sushibta.minita.final"):nil;if(![v isKindOfClass:UIView.class]){MTLog(@"[FINAL-HOST] no presentation=%@",v);return;}UIWindowScene *ws=MTCarScene();if(!ws){MTLog(@"[FINAL-HOST] no CarPlay UIWindowScene");return;}if(!gHostWindow){gHostWindow=[[UIWindow alloc]initWithWindowScene:ws];gHostWindow.frame=ws.coordinateSpace.bounds;gHostWindow.windowLevel=UIWindowLevelAlert+60;gHostWindow.rootViewController=[UIViewController new];gHostWindow.rootViewController.view.backgroundColor=UIColor.blackColor;}UIView *p=(UIView*)v;[p removeFromSuperview];p.frame=gHostWindow.bounds;p.autoresizingMask=UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;[gHostWindow.rootViewController.view addSubview:p];gPresentation=p;gHostWindow.hidden=NO;MTLog(@"[FINAL-HOST] ATTACHED class=%@ frame=%@ subviews=%lu window=%@",NSStringFromClass(p.class),NSStringFromCGRect(p.frame),(unsigned long)p.subviews.count,p.window);dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(1.0*NSEC_PER_SEC)),dispatch_get_main_queue(),^{MTLog(@"[FINAL-HOST+1] scene=%@ process=%@ foreground=%@ presentation=%@ subviews=%lu window=%@",MTV(gYTController,@"scene"),MTV(MTV(gYTController,@"scene"),@"clientProcess"),MTV(MTV(gYTController,@"scene"),@"settings"),gPresentation,(unsigned long)gPresentation.subviews.count,gPresentation.window);});}@catch(NSException*e){MTLog(@"[FINAL-HOST] ERROR %@ %@",e.name,e.reason);}});}
